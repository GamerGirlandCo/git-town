...
func addDescendantNodes(branch gitdomain.LocalBranchName, args ProposalStackLineageArgs, visited map[gitdomain.LocalBranchName]*ProposalStackLineageTreeNode, tree *ProposalStackLineageTree) error {
	...
	if proposal, ok := tree.BranchToProposal[branch]; ok {
		...
		if proposal.IsNone() {
			ancestors := args.Lineage.Ancestors(branch).BranchNames()
			proposal, err = findProposal(branch, gitdomain.LocalBranchName(ancestors[0]), args.Connector)
			if err != nil {
				return err
			}
		}
...
func findRelevantChildren(
	...
	for _, child := range children {
		if shouldIncludeChild(ancestor, child, args.MainAndPerennialBranches, ancestors) {
			...
			// fallback to searching for proposals with the branch lineage's root as the base branch	
			if proposal.IsNone() {
				proposal, err = findProposal(child, ancestors[0], args.Connector)
				if err != nil {
					return nil, err
				}
			}
